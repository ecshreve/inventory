...
// QueryStorageLocation chains the current query on the "storage_location" edge.
func (iq *ItemQuery) QueryStorageLocation() *StorageLocationQuery {
	query := (&StorageLocationClient{config: iq.config}).Query()
...
func (iq *ItemQuery) Clone() *ItemQuery {
	...
	return &ItemQuery{
		...
		withStorageLocation: iq.withStorageLocation.Clone(),
...
// the "storage_location" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *ItemQuery) WithStorageLocation(opts ...func(*StorageLocationQuery)) *ItemQuery {
	query := (&StorageLocationClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withStorageLocation = query
	return iq
}
...
func (iq *ItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Item, error) {
	var (
		...
		_spec       = iq.querySpec()
		loadedTypes = [1]bool{
			iq.withStorageLocation != nil,
		}
	)
	if iq.withStorageLocation != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := iq.withStorageLocation; query != nil {
		if err := iq.loadStorageLocation(ctx, query, nodes, nil,
			func(n *Item, e *StorageLocation) { n.Edges.StorageLocation = e }); err != nil {
			return nil, err
		}
	}
...
func (iq *ItemQuery) loadStorageLocation(ctx context.Context, query *StorageLocationQuery, nodes []*Item, init func(*Item), assign func(*Item, *StorageLocation)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Item)
	for i := range nodes {
		if nodes[i].item_storage_location == nil {
			continue
		}
		fk := *nodes[i].item_storage_location
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "item_storage_location" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
