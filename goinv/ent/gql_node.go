...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case storagelocation.Table:
		query := c.StorageLocation.Query().
			Where(storagelocation.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, storagelocationImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case storagelocation.Table:
		query := c.StorageLocation.Query().
			Where(storagelocation.IDIn(ids...))
		query, err := query.CollectFields(ctx, storagelocationImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
