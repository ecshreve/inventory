...
// QueryStorageLocation queries the storage_location edge of a Item.
func (c *ItemClient) QueryStorageLocation(i *Item) *StorageLocationQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, item.StorageLocationTable, item.StorageLocationColumn),
		)
...
// UpdateOne returns an update builder for the given entity.
func (c *StorageLocationClient) UpdateOne(sl *StorageLocation) *StorageLocationUpdateOne {
	mutation := newStorageLocationMutation(c.config, OpUpdateOne, withStorageLocation(sl))
	return &StorageLocationUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *StorageLocationClient) UpdateOneID(id int) *StorageLocationUpdateOne {
...
// DeleteOne returns a builder for deleting the given entity.
func (c *StorageLocationClient) DeleteOne(sl *StorageLocation) *StorageLocationDeleteOne {
	return c.DeleteOneID(sl.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *StorageLocationClient) DeleteOneID(id int) *StorageLocationDeleteOne {
...
func (c *StorageLocationClient) Get(ctx context.Context, id int) (*StorageLocation, error) {
	return c.Query().Where(storagelocation.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *StorageLocationClient) GetX(ctx context.Context, id int) *StorageLocation {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryItems queries the items edge of a StorageLocation.
func (c *StorageLocationClient) QueryItems(sl *StorageLocation) *ItemQuery {
...
type (
	hooks struct {
		Item, StorageLocation []ent.Hook
	}
	inters struct {
		Item, StorageLocation []ent.Interceptor
	}
)
