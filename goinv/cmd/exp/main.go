...
func NewModel() Model {
	...
	inv, _ := goinv.NewGormInventory()
	invItems, _ := inv.GetItems()

	listItems := make([]list.Item, 0, len(invItems))
	for _, invItem := range invItems {
		listItems = append(listItems, item(invItem))
	}

	m.list = list.New(listItems, list.NewDefaultDelegate(), 40, 20)

	m.form = huh.NewForm(
		huh.NewGroup(
			huh.NewSelect[string]().
				Key("category").
				Options(huh.NewOptions(goinv.AllCategories...)...).
				Title("Choose item category"),

			huh.NewSelect[string]().
				Key("location").
				Options(huh.NewOptions(goinv.AllLocations...)...).
...
func (m Model) View() string {
	...
	default:
		...
		{
			var (
				cat = m.form.GetString("category")
				loc = m.form.GetString("location")
			)
